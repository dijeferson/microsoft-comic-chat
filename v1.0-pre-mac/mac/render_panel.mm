...
//
// Usage: render_panel <avatarDir> <name> "<text>" <out.png> [backdropDir/name.bmp]
//   The optional 5th positional (argv[5]) is a full path to a backdrop .bmp; if
//   given, it is loaded and drawn as the panel background.
...
int main(int argc, const char* argv[]) {
    if (argc != 5 && argc != 6) {
        fprintf(stderr, "usage: %s <avatarDir> <name> <text> <out.png> [backdrop.bmp]\n", argv[0]);
        return 2;
    }
    @autoreleasepool {
        std::string dir = argv[1], name = argv[2], text = argv[3];
        NSString* out = [NSString stringWithUTF8String:argv[4]];
        std::string backdropPath = (argc == 6) ? argv[5] : "";
